...
export class UsersService {
  ...
  async signin(
  ...
  ): Promise<{ message: string; user: UserEntity; isFirstLogin: boolean; smartMeterToken: string }> {
    const userExists = await this.usersRepository
      .createQueryBuilder('users')
      .addSelect('users.password')
      .where('users.email = :email', { email: userSignInDto.email })
    ...
    const smartMeterToken = await this.smartMeterService.getToken();

    if (!smartMeterToken) {
      throw new UnauthorizedException('Unable to retrieve Smart Meter token');
    }

    delete userExists.password;
    return { message: 'Login Successful', user: userExists, smartMeterToken , isFirstLogin: userExists.isFirstLogin };
  }
